import type { SupportingResourceRelationship } from "../../types/content";
import { ARTIFACT_IDS } from "./artifacts";

const PRIMARY_ID = ARTIFACT_IDS.bellabeat;
...
export const supportingRelationships = SUPPORT_TARGETS.map(
  ([targetResourceId, role], index) => ({
    id: `relationship-bellabeat-support-${String(index + 1).padStart(2, "0")}`,
    sourceResourceId: PRIMARY_ID,
    targetResourceId,
...
) satisfies readonly SupportingResourceRelationship[];
