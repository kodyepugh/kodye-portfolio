...
} from "@/lib/reservoir/direct-resource-routing";
...
function getInitialCollectionId(initialRoute: ReservoirSceneProps["initialRoute"]) {
  return initialRoute.kind === "collection" ||
    initialRoute.kind === "contextual-resource"
    ? initialRoute.collectionId
    : ROOT_COLLECTION_ID;
}

function createRouteRestorationDescriptor(
  route: ReservoirSceneProps["initialRoute"],
  entryId: string,
): PublicRouteRestorationDescriptor {
  ...
  const withContext = (context: ReservoirContext) =>
    getReservoirContextKey(context) === getReservoirContextKey(rootContext)
      ? [rootFrame]
      : [
          rootFrame,
          {
  ...
  const resourceExistsAtRoot = getReservoirContextNodes(rootContext).some(
    (node) => node.kind !== "collection" && node.id === route.resourceId,
  );
  return {
...
export function ReservoirScene({ initialRoute }: ReservoirSceneProps) {
  const initialCollectionId = getInitialCollectionId(initialRoute);
  ...
  >(
    () =>
      new Set(
        getReservoirContentNodes(initialCollectionId).map((node) => node.id),
      ),
  );
  ...
  >(() => {
    const rootFrame: ReservoirHistoryFrame = {
      ...
      label: getCollectionById(ROOT_COLLECTION_ID)?.title ?? "Home",
    };
    if (initialCollectionId === ROOT_COLLECTION_ID) return [rootFrame];
    return [
      rootFrame,
      {
        id: "reservoir-visit-1",
        context: { kind: "collection", collectionId: initialCollectionId },
        label: getCollectionById(initialCollectionId)?.title ?? "Collection",
      },
    ];
  });
  ...
  const inspectedResourceIdRef = useRef(inspectedResourceId);
  ...
  const pendingAutoOpenInspectionResourceIdRef = useRef<string | null>(null);
  ...
  const pendingInspectionReturnFrameRef =
    useRef<InspectionReturnFrame | null>(null);
  const pendingContextualResourceFocusRef =
    useRef<ContextualResourceFocusIntent | null>(null);
  const focusActiveReservoirResourceRef = useRef<
    (resourceId: string, origin: FocalInspectionOrigin) => boolean
  >(() => false);
      ...
      window.history.replaceState(
        {
          ...
          ...createPublicRouteHistoryEntry({
            ...
            initial: currentEntry.initial,
            restoration: {
              reservoirHistory: history,
              inspectedResourceId,
            },
            returnPath: currentEntry.returnPath,
            closeAction: currentEntry.closeAction,
          }),
        },
        "",
        currentEntry.path,
  ...
  const commitReservoirHistory = useCallback(
    (history: ReservoirHistoryFrame[]) => {
      ...
      const autoOpenInspectionResourceId =
        pendingAutoOpenInspectionResourceIdRef.current;
      ...
      if (suppressNextBrowserHistoryWriteRef.current) {
        ...
        pendingAutoOpenInspectionResourceIdRef.current = null;
        return;
      }
      if (window.location.pathname !== nextPath) {
        const returnPath =
          nextContext.kind === "query"
            ...
            : undefined;
        const entryId = createBrowserEntryId();
        window.history.pushState(
          createPublicRouteHistoryEntry({
            ...
            restoration: {
              reservoirHistory: history,
              inspectedResourceId: autoOpenInspectionResourceId,
            },
            returnPath,
            closeAction:
        ...
        setActiveBrowserEntryId(entryId);
      }
      pendingAutoOpenInspectionPathRef.current = null;
      pendingAutoOpenInspectionResourceIdRef.current = null;
    },
  ...
  useEffect(() => {
    inspectedResourceIdRef.current = inspectedResourceId;
  }, [inspectedResourceId]);
  useEffect(() => {
  ...
  const inspectionReturnFramePresent =
    activeInspectionReturnFrame !== null || inspectionReturnRuntimeInProgress;
  const inspectionReturnDiagnosticFrame =
    inspectionReturnRuntimeInProgress
      ? inspectionReturnRuntime?.frame ?? null
      : activeInspectionReturnFrame ?? inspectionReturnRuntime?.frame ?? null;
  ...
  requestInspectionCloseRef.current = requestInspectionClose;
  ...
  const requestInspectionNavigation = useCallback(
    (resourceId: string, returnFrame: InspectionReturnFrame) => {
      ...
      if (returnFrame.resourceId !== inspectedResourceId) {
        setInspectionReturnRuntime({
          queryContextKey: "",
          returnContextKey: getReservoirContextKey(settledReservoirContext),
          frame: returnFrame,
          phase: "failed",
        });
        return;
  ...
  const requestInspectionCollectionNavigation = useCallback(
    (collectionId: string, returnFrame: InspectionReturnFrame) => {
      ...
      pendingInspectionCollectionReturnFrameRef.current = returnFrame;
      setInspectionReturnRuntime({
        queryContextKey: "",
        returnContextKey: getReservoirContextKey(settledReservoirContext),
        frame: returnFrame,
  ...
  useEffect(() => {
    ...
    function updateClosingRecovery(now: number) {
      ...
      const supportNavigationTarget =
        pendingInspectionNavigationTargetRef.current ?? null;
      const supportReturnFrame = pendingInspectionReturnFrameRef.current;
      ...
      if (supportHandoffReady && supportNavigationTarget) {
        ...
        setInspectionReturnRuntime((currentRuntime) =>
          currentRuntime?.phase === "captured"
            ? { ...currentRuntime, phase: "detouring" }
            : currentRuntime,
        );
        if (interaction.current) {
  ...
  useEffect(() => {
    ...
    const inspectionReturnFrame =
      pendingInspectionCollectionReturnFrameRef.current;
  ...
  focusActiveReservoirResourceRef.current = focusActiveReservoirResource;
  ...
  useEffect(() => {
    const pendingContextualResourceFocus =
      pendingContextualResourceFocusRef.current;
    if (
      !pendingContextualResourceFocus ||
      transitionState !== "idle" ||
      collectionNavigation.transitionPhase !== "idle" ||
      layoutOwnership.transitionPlan !== null
    ) {
    ...
    const { resourceId, collectionId } = pendingContextualResourceFocus;
    const activeContext = queryReservoirContext ?? collectionReservoirContext;
    if (
      activeContext.kind !== "collection" ||
      activeContext.collectionId !== collectionId
    ) {
      return;
    }

    pendingContextualResourceFocusRef.current = null;
    if (!focusActiveReservoirResourceRef.current(resourceId, "route")) {
      if (interaction.current) {
        interaction.current.dataset.directResourceRequestResult =
          "rejected-contextual-active-reservoir";
      }
    }
  }, [
    collectionNavigation.transitionPhase,
  ...
  function requestDirectResource(
    ...
    if (!resource || resource.published !== true) {
      ...
      if (inspectionReturnFrame) {
        setInspectionReturnRuntime((currentRuntime) => ({
          queryContextKey: currentRuntime?.queryContextKey ?? "",
          returnContextKey:
            currentRuntime?.returnContextKey ??
            getReservoirContextKey(settledReservoirContext),
          frame: inspectionReturnFrame,
          phase: "failed",
        }));
    ...
    const returnContext =
      returnContextOverride ?? queryReservoirContext ?? collectionReservoirContext;
    const activeResourceReuseEligible = canReuseActiveReservoirResource({
      resourceId: resource.id,
      activeContext: renderedReservoirContext,
      activeResourceIds: activeReservoirResources.map((node) => node.id),
      requestedCollectionId: requestedCollectionId,
    });

    if (preferActiveReservoir && openInspection && activeResourceReuseEligible) {
    ...
    if (
      preferActiveReservoir &&
      openInspection &&
      requestedCollectionId &&
      returnContext.kind === "collection" &&
      !(
        renderedReservoirContext.kind === "collection" &&
        renderedReservoirContext.collectionId === requestedCollectionId
      )
    ) {
      const currentHistory = reservoirHistoryRef.current;
      const currentVisit = currentHistory.at(-1);
      ...
      const historyWithTargetContext = resolveContextualResourceHistory(
        currentHistory,
        requestedCollectionId,
        createReservoirHistoryFrame,
      );
      if (!historyWithTargetContext) return false;
      const historyWithInspectionReturn = inspectionReturnFrame
        ? setInspectionReturnForReservoirVisit(
            historyWithTargetContext,
            currentVisit.id,
            inspectionReturnFrame,
          )
        : historyWithTargetContext;
      ...
      if (interaction.current) {
        interaction.current.dataset.directResourceRequest = resourceAddress;
        interaction.current.dataset.directResourceRequestResult =
          "active-reservoir-after-contextual-reconstitution";
        interaction.current.dataset.directResourceRequestMode =
          "contextual-collection-reconstitution";
        interaction.current.dataset.directResourceResolvedId = resource.id;
        interaction.current.dataset.directResourceArtifactStatus = String(
          resource.isArtifact,
        );
    ...
    pendingAutoOpenInspectionPathRef.current = openInspection
      ...
      : null;
    pendingAutoOpenInspectionResourceIdRef.current = openInspection
      ? resource.id
      : null;
    const currentHistory = reservoirHistoryRef.current;
    const currentVisit = currentHistory.at(-1);
    ...
    if (!requestQueryReservoirContext(
      ...
      if (inspectionReturnFrame) {
        setInspectionReturnRuntime({
          queryContextKey: getReservoirContextKey(targetContext),
  ...
  useEffect(() => {
    const currentEntry = getPublicRouteHistoryEntry(window.history.state);
    if (currentEntry?.path === window.location.pathname) {
      window.queueMicrotask(() => setActiveBrowserEntryId(currentEntry.entryId));
      return;
    }
    const entryId = createBrowserEntryId();
    const existingState =
    ...
    window.history.replaceState(
      {
        ...
        ...createPublicRouteHistoryEntry({
          ...
          initial: true,
          restoration: createRouteRestorationDescriptor(
            initialRoute,
            entryId,
          ),
        }),
    ...
    window.queueMicrotask(() => setActiveBrowserEntryId(entryId));
  ...
  useEffect(() => {
    function visitBrowserRoute(event: PopStateEvent) {
      ...
      let entry = getPublicRouteHistoryEntry(event.state);
      if (!entry) {
        const entryId = createBrowserEntryId();
        entry = createPublicRouteHistoryEntry({
          ...
          path: routePath,
          initial: true,
          restoration: createRouteRestorationDescriptor(route, entryId),
        });
        const existingState =
      ...
      const intent = createBrowserRestorationIntent(
        entry,
        browserRestorationRevisionRef.current,
      );
  ...
  useEffect(() => {
    ...
    function processBrowserRestoration() {
      ...
      const finishRestoration = (phase: "converged" | "failed") => {
        suppressNextBrowserHistoryWriteRef.current = false;
        pendingInspectionCloseVisitIdRef.current = null;
      ...
      if (inspectedResourceId !== null) {
        ...
        const targetReadingStateActive =
          ...
          inspectionReturnRuntime?.frame.resourceId === targetResourceId &&
          (inspectionReturnRuntime.phase === "reopening-inspection" ||
        ...
        if (transitionState === "readingInspection") {
          pendingInspectionCloseVisitIdRef.current = null;
          pendingInspectionClosePathRef.current = window.location.pathname;
          pendingInspectionCloseBrowserActionRef.current = "none";
      ...
      if (!contextMatches) {
        ...
        const started =
          targetContext.kind === "collection"
            ? requestCollectionRef.current(targetContext.collectionId)
            : requestQueryReservoirContextRef.current(targetContext);
        if (!started) {
          pendingReservoirHistoryResolutionRef.current = null;
          finishRestoration("failed");
          return;
      ...
      if (!focusActiveReservoirResourceRef.current(targetResourceId, "route")) {
        updateDiagnostics("deferred", "resource-focus-not-ready");
        return;
      }
      updateDiagnostics("opening-inspection");
    }
  ...
  useEffect(() => {
    ...
    const existingState =
      window.history.state && typeof window.history.state === "object"
        ...
        : {};
    const currentEntry = getPublicRouteHistoryEntry(window.history.state);
    const entryId = currentEntry?.entryId ?? createBrowserEntryId();
    window.history.replaceState(
      {
        ...existingState,
        ...createPublicRouteHistoryEntry({
    ...
    setActiveBrowserEntryId(entryId);
  }, [createBrowserEntryId, inspectedResourceId, reservoirHistory, transitionState]);
  ...
  useEffect(() => {
    ...
    const settledContextKey = getReservoirContextKey(settledReservoirContext);
    if (settledContextKey !== inspectionReturnRuntime.returnContextKey) {
      return;
    }
    ...
    const resourceExistsInReturnedContext = getReservoirContextNodes(
      ...
      (node) =>
        ...
        node.id === inspectionReturnRuntime.frame.resourceId,
    );
    ...
    setHoveredResourceId(null);
    setSelectedCollectionId(null);
    setSelectedResourceId(storedFrame.resourceId);
    setSelectedPressActive(false);
    const inspectionStarted = beginResourceInspectionRef.current(
    ...
    setInspectionReturnRuntime((currentRuntime) =>
      currentRuntime?.phase === "returning-reservoir"
        ? { ...currentRuntime, frame: storedFrame, phase: "reopening-inspection" }
        : currentRuntime,
    );
  }, [
    collectionNavigation.transitionPhase,
  ...
  return (
    <>
      ...
      data-inspection-return-frame-present={inspectionReturnFramePresent}
      data-inspection-return-context-key={inspectionReturnDiagnosticContextKey}
      data-inspection-return-resource-id={
        inspectionReturnDiagnosticFrame?.resourceId ?? ""
      }
      data-inspection-return-scroll-y={
        inspectionReturnDiagnosticFrame?.scrollY.toFixed(3) ?? ""
      }
      data-inspection-return-post-content-progress={
        inspectionReturnDiagnosticFrame?.postContentProgress.toFixed(6) ?? ""
      }
      data-inspection-return-phase={inspectionReturnDiagnosticPhase}
    ...
    >
      ...
      {inspectionWindowPhase && openingResource ? (
        <InspectionWindow
          ...
          initialReturnFrame={
            inspectionReturnRuntime?.phase === "reopening-inspection" &&
            inspectionReturnRuntime.frame.resourceId === openingResource.id
